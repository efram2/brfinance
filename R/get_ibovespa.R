...
#'   - `NULL` defaults to one year before the current date
#' @param end_date End date for the data period. Accepts the same formats as
#'   `start_date`. `NULL` defaults to the current date (today).
...
get_ibovespa <- function(start_date = NULL,
  ...
  # Default window: last 12 months, since (unlike BCB/SGS defaults) pulling
  # the full 1986-onward history from Yahoo on every call would be slow and
  # is rarely what's wanted for an equity index.
